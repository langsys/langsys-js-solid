...
| Suite                       | **67 tests / 8 files**, all passing — 64 browser-build + 3 SSR-build                                                                            |
...
| **BIND-5** — does not cache lookup results                        | **`open`**    | No memo primitive exists (`createMemo\|useMemo\|cache(` → **0**). But `useT()` sits behind **two equality gates in series**: the core's `Object.is` guard (`signal.ts:43`) and Solid's `createSignal` default `===` inside `useSignal` (`adapters.ts:26`). Both are inert **only** because `buildTFn()` mints a fresh identity per emit (`translations.ts:97-99`). Now proved **at the DOM**, not just in a computation: with a stable identity a rendered component's text stays frozen across two emits while the positive control repaints (`rendered.test.tsx`). Routed to the Typescript lane. Not graded until ruled. |
...
- The BIND-5 hazard, at the DOM: a stable `TFunction` identity leaves rendered text **frozen at
  `v1` across two emits**, while the fresh-identity control repaints. The `open` row is no longer
...
Corrected in `adapters.ts` and the README, and pinned by three assertions in `rendered.test.tsx`
that run the real `canonicalizeLocale` rather than restating the rule. The store itself is
unchanged: it round-trips input verbatim, because normalizing in the binding would be the binding
adapting meaning (BIND-1) and would diverge from the core the moment its rule changed.
...
npm test                            # 67 tests / 8 files — runs BOTH projects
