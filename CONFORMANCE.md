...
| **Suite**              | **109 tests / 12 files**, all passing: 99 browser-build + 10 SSR-build (typecheck, test and build each exit 0)                                                                                                                                                                                                                                   |
| **Core consumed**      | `langsys-js-typescript` at `f58e0c43593108fc42b33524e634ac13c94fbbee` through `npm link`, src clean at probe time; dist rebuilt 2026-09-12T18:14:07 from that clean tree. `package.json` declares `^0.6.5`, the core version built against; it must match the core version that ships in the release wave (see [The two 0.6.5s](#the-two-065s)). |
| **Tally**              | 59 delegated, 13 implemented, 6 not implemented, 1 n/a (profile)                                                                                                                                                                                                                                                                                 |

**What surfaced while writing this:**

- **All three components crashed a server render, and the first report named only two.** Rendered
  under Solid's server build, `<Translate>`, `<Phrase>` and `<DontTranslate>` each threw
  `ReferenceError: document is not defined`, failing the whole SSR response. The first measurement
  missed `<DontTranslate>`. Fixed in this revision on the operator's approval: under a server render
  they now emit their markup untranslated, so the page degrades to the base language. The SRV rules
  stay unmet.
- **The fix shipped a parity bug on its first try.** The server host emitted `class=""` whenever no
  class was given, while the browser path sets no attribute at all: a server/client disagreement in
  exactly the markup hydration has to match. Caught by a test that looked for it, fixed by dropping
  absent attributes, and pinned by a row that goes red if they come back.
- **A mutation measured against a red baseline.** The first guard-removal run was taken while one server
  row was already failing on a brittle assertion (Solid's server build pads the value, `class="hero "`),
  so its count proved nothing about the guard. Re-run from a green baseline.
...
| SSR-1   | delegated             | -          | Core row SSR-1 (core-graded: provisional). Strategy selection and server-side collection are the core's; this binding's `isServer` guards are lifecycle, not strategy (see Probes). Capability markup under SSR is separately pinned undecided in `src/capability.ssr.test.tsx` (BIND-1). probe SSRSTRAT mine 0 / core 10 (firing control).                                                                                                                                                                                                                                                                                                                                                                            |
| SSR-2   | delegated             | -          | Core row SSR-2 (core-graded: provisional). probe SSRSTRAT mine 0 / core 10 (firing control).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| SSR-3   | delegated             | -          | Core row SSR-3 (core-graded: provisional, no test). probe SSRSTRAT mine 0 / core 10 (firing control).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| SRV-1   | not implemented       | -          | Measured under Solid's server build: with no seed, `t()` serves the base language; with a hand-seeded catalog it serves the translation, but only through the core's process-global. `<Translate>`, `<Phrase>` and `<DontTranslate>` used to throw `ReferenceError: document is not defined`, failing the whole response; they now emit their markup untranslated (`src/components.ssr.test.tsx`), so a server-rendered page degrades to the base language instead of erroring. That is lifecycle adaptation, not this rule: nothing here serves the request locale's translations, and whether the client hydrates cleanly against that markup is unproven.                                                           |
...
| SRV-5   | not implemented       | -          | The components no longer throw under a server render (`src/components.ssr.test.tsx`), but nothing captures their children on the server and nothing fails with a named error on a child it cannot capture: children are emitted untranslated. Once-per-subtree counting is untested. Where server-side child capture lives is an open operator decision.                                                                                                                                                                                                                                                                                                                                                               |
| BIND-1  | implemented           | n/a (pure) | Adaptation is confined to Solid's execution model. `src/capability.test.ts` and `src/capability.ssr.test.tsx`: capability seeds `undefined` and adopts the core value only after mount (mutation: seed eagerly, 2 red in each project). `src/write-grant.test.ts`: the grant is resolved per call (mutation: snapshot at adapt time, 3 red). `src/rendered.test.tsx`: the locale store round-trips input verbatim. `src/components.ssr.test.tsx`: under a server render the three components emit their markup untranslated instead of throwing, and set no attribute the browser path would not (mutations: remove the guard from Translate, Phrase, DontTranslate, 2, 4 and 2 red; spread absent attributes, 1 red). |
...
| WIRE4    | 0    | 26           | WIRE-4                                                                                     |
| CAT      | 0    | 12           | CAT-1..3                                                                                   |
| SSR      | 6    | 23           | retired from SSR-1..3 and CONF-3: every hit is one of this binding's own `isServer` guards |
| SSRSTRAT | 0    | 10           | CONF-3, SSR-1..3                                                                           |
| GRANT3   | 0    | 4            | GRANT-3                                                                                    |
| OBS      | 0    | 12           | OBS-1                                                                                      |
| SRVCOL   | 0    | 31           | SRV-3                                                                                      |
| MEMO     | 0    | 3            | BIND-5                                                                                     |
...
**One probe re-scoped, with its reason (2026-09-12).** `SSR` returned mine 0 until this revision added
`isServer` guards to the three components; it now returns 6, and every hit is one of those guards. They
are lifecycle adaptation, not SSR strategy participation, so SSR-1..3 and CONF-3 now use `SSRSTRAT`
(`ssrTokenStrategy`), mine 0 / core 10. A probe that starts matching the binding's own new code is
re-scoped with its reason recorded, not quietly loosened.
...
| Guard removed from `<Translate>` (`if (isServer)` disabled)                                         | server-render guard          | **2 failed**: the Translate server row and `emits no class attribute when none is given`.                                                                                                                                         |
| Guard removed from `<Phrase>`                                                                       | server-render guard          | **4 failed**: the Phrase server row, the class-parity row and its control, and the tag-prop row.                                                                                                                                  |
| Guard removed from `<DontTranslate>`                                                                | server-render guard          | **2 failed**: the DontTranslate server row and the class-parity row.                                                                                                                                                              |
| `serverHost` spreads absent attributes again                                                        | server/client parity         | **1 failed**: `emits no class attribute when none is given`; its control stays green.                                                                                                                                             |
| The same guard mutation, first taken against a red baseline                                         | harness                      | **Void, and discarded**: the Translate server row was already failing on a brittle class assertion (`class="hero "`), so the count proved nothing about the guard. Re-run above from a green baseline.                            |
...
The server-render crash that led this list in the previous revision is fixed: the components now degrade
to the base language (see What surfaced). What remains:

1. **SRV-1, SRV-2 — served HTML is the base language, and the catalog is process-global.** Crawlers index
...
2. **SRV-4 — no pre-hydration seed is proven, and hydration against server markup is untested.** The
   core's synchronous `seedCatalog` is reachable here, but nothing shows a client entry calling it before
   hydration, and nothing tests the components' client-built hosts against the markup the server now
   emits. A page served in one language may re-render in another.
3. **SRV-5 — no server-side child capture.** The components emit their children untranslated rather than
   capturing them or failing with a named error. Blocked on the open ruling on where child capture lives.
4. **HINT-4 — persistent layouts never capture the new route.** Measured: 1 of 2 components re-enters `t()`
   across a navigation. Content in a persistent layout is silently never discovered for later routes.
   Closes with the navigation entry point.
...
The component half of the SRV measurements is now committed as `src/components.ssr.test.tsx`, because it
pins fixed behaviour: the three components render under the server build without throwing. The `t()` half
is reproduced by rendering under the server project (`vitest.ssr.config.mts`) with `renderToString`:
`useT()` inside a component with no catalog seeded; the same after `sTranslations.set` with an Italian
catalog; then a German catalog seeded and the Italian render repeated. That half stays uncommitted, since a
test pinning today's behaviour there would pin a defect.
