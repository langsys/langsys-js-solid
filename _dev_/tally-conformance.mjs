#!/usr/bin/env node
/**
 * ADAPTED FOR A BINDING REPO from langsys-js-typescript's `_dev_/tally-conformance.mjs`
 * (the core lane's checker, itself built to the shape the reviewer-side checker measures).
 * Two changes, and only these:
 *   1. a binding's Profiles row carries a "— derived: binding over …" suffix, so only the
 *      words before " — " are read as profiles;
 *   2. this repo's tests live under src/ as *.test.ts and *.test.tsx, and are matched by the
 *      basename of any backticked path in the evidence cell.
 * Run from the repo root:  node _dev_/tally-conformance.mjs ~/Documents/dev/langsys2
 */
/**
...
for (const row of rows) {
    ...
    if (!/^[A-Z]{2,5}-\d+$/.test(row.id)) {
        err(
            row,
            'the Rule cell must be exactly one rule id; no ranges, families, lists, wildcards or descriptive suffixes'
        );
        continue;
    }
...
const walk = (d) =>
    existsSync(d)
        ...
        : [];
const testsOnDisk = new Set(walk(join(ROOT, 'src')).filter((f) => /\.test\.tsx?$/.test(f)));
const namesATest = (text) =>
    [...text.matchAll(/`([^`]+)`/g)].some((m) => testsOnDisk.has(m[1].split('/').pop().split('::')[0]));

function classify(status) {
    let m;
    if (status === 'implemented')
        return { key: status, green: true, tiers: ['live', 'contract', 'n/a (pure)'], needsTest: true };
    ...
    if ((m = status.match(/^n\/a \(profile: (.+)\)$/)))
        return { key: 'n/a (profile)', green: true, tiers: ['-'], profileClaim: m[1] };
    if ((m = status.match(/^n\/a \(architecture: (.{12,})\)$/)))
        return { key: 'n/a (architecture)', green: true, tiers: ['-'] };
    return null;
}
...
for (const row of rows) {
    ...
    if (!g) {
        err(
            row,
            `status "${row.status}" is not canonical (implemented | provisional | delegated | partial | ` +
    ...
    if (!TIERS.includes(row.tier)) err(row, `tier "${row.tier}" is not one of: ${TIERS.join(' | ')}`);
        ...
        const why =
            g.key === 'implemented' && row.tier === 'mock' ? ' (mock evidence makes the row provisional, CONF-2)' : '';
    ...
    if (g.bindingOnly && !sdkProfiles.includes('binding'))
        err(row, 'delegated is for bindings only, and this SDK does not claim the binding profile');
...
if (!structureOnly && citedBlob && citedCommit) {
    if (!existsSync(join(SPEC_REPO, '.git')))
        cannotCheck(`no spec checkout at ${SPEC_REPO} (pass it, or use --structure-only)`);
    ...
    if (carried !== citedBlob) {
        ...
        anchors.forEach((a, i) => {
            ...
            profiles.set(
                a[1].toUpperCase(),
                (body.slice(a.index, end).match(/\*\*Profiles:\*\*\s*(.+)/)?.[1] ?? '').trim()
            );
        });
        const words = (p) =>
            new Set(
        ...
        for (const id of profiles.keys())
            if (!seen.has(id)) errors.push(`MISSING  ${id} has no row (Profiles: ${profiles.get(id)})`);
        for (const [id, line] of seen)
            if (!profiles.has(id)) errors.push(`:${line}  ${id} is not a rule in blob ${citedBlob}`);
        for (const row of rows) {
            if (row.grade?.key === 'n/a (profile)' && profiles.has(row.id) && binds(profiles.get(row.id))) {
                err(
                    row,
                    `n/a (profile) but its Profiles line binds a profile this SDK claims: ${profiles.get(row.id)}`
                );
            }
        }
    }
...
const waits = count((r) =>
    r.grade?.key === 'provisional' ? (r.evidence.match(/waits on:\s*([^;.|]+)/i)?.[1] ?? '?').trim() : null
);
waits.delete(null);
...
console.log(
    spec
        ? `spec   : ${spec.profiles.size} rule ids in the cited blob`
        : 'spec   : not consulted (--structure-only, or no readable citation)'
);
...
if (notGreen.length) {
    ...
    for (const r of notGreen)
        console.log(`  :${String(r.line).padEnd(5)}${String(r.id).padEnd(10)}${r.grade?.key ?? r.status}`);
}
if (errors.length) {
